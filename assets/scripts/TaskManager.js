...
function renderTasks(projectId) {
    ...
    projectTasks.forEach(task => {
      ...
      deleteButton.innerHTML = '<img src="assets/icons/deleteIcon.png" alt="Edit" style="width: 20px; height: 20px;">';
      ...
      editButton.innerHTML = '<img src="assets/icons/gearIcon.png" alt="Edit" style="width: 20px; height: 20px;">';
      let isEditing = false;
      editButton.onclick = function() {
          if (isEditing) {
              nameField.readOnly = true;
              dateField.readOnly = true;
              editButton.innerHTML = '<img src="assets/icons/gearIcon.png" alt="Edit" style="width: 20px;">';
              // Update display colors or other changes based on new date
              const newDateDiff = (new Date(dateField.value) - new Date()) / (1000 * 60 * 60 * 24);
              taskDiv.style.backgroundColor = `rgb(${getColorForDays(newDateDiff).join(',')})`;
...
function addTaskToProject() {
    ...
    if (project) {  // If project found, add the new task to its tasks array
...
function updateDateColor() {
    this.style.backgroundColor = `rgb(${getColorForDays((new Date(this.value) - new Date()) / (1000 * 60 * 60 * 24)).join(',')})`;
}
...
function deleteTask(taskId) {
    ...
    const project = projects.find(p => p.id === Number(document.getElementById('viewCardModal').dataset.id));
